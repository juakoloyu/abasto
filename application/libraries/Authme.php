...
class Authme {
	...
	{
		$user = $this->CI->authme_model->get_user_by_dni($dni);
		if($user){
	...
	public function signup($password, $nombre, $apellido, $dni)
	{
		$password = $this->PasswordHash->HashPassword($password);
		$this->CI->authme_model->create_user($password, $nombre, $apellido, $dni);
		return true;
	}
